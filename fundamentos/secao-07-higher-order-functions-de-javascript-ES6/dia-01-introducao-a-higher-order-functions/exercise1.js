// 1
const generateEmployee = (fullName) => {
  const email = fullName.toLowerCase().replace(" ", "_");
  return { name: fullName, email: `${email}@trybe.com` };
};

const newEmployees = (callback) => {
  const employees = {
    id1: callback("Pedro Guerra"), // Nome: Pedro Guerra -> Chame sua função passando o nome Pedro Guerra como parâmetro, substituindo as aspas
    id2: callback("Luiza Drumond"), // Nome: Luiza Drumond -> Chame sua função passando o nome Luiza Drumond como parâmetro, substituindo as aspas
    id3: callback("Carla Paiva"), // Nome: Carla Paiva -> Chame sua função passando o nome Carla Paiva como parâmetro, substituindo as aspas
  };
  return employees;
};

// 2
const confereNumeros = (num1, num2) => {
  const ganhou = "Parabéns, você ganhou";
  const perdeu = "Tente novamente";
  return num1 === num2 ? ganhou : perdeu;
};

const resultadoSorteio = (numeroApostado, callback) => {
  const numeroSorteio = Math.floor(Math.random() * 5) + 1;
  return callback(numeroApostado, numeroSorteio);
};

// 3
const compareAnswer = (rightAnswer, studentAnswer) => {
  let points = 0;
  rightAnswer.forEach((question, index) => {
    if (studentAnswer[index] !== 'N.A') {
      points += question === studentAnswer[index] ? 1 : -0.5;
    }
  })
  return points;
};

const RIGHT_ANSWERS = ['A', 'C', 'B', 'D', 'A', 'A', 'D', 'A', 'D', 'C'];
const STUDENT_ANSWERS = ['A', 'N.A', 'B', 'D', 'A', 'C', 'N.A', 'A', 'D', 'B'];

const returnPoints = (rightAnswers, studentAnswers, callback) => {
  return callback(rightAnswers, studentAnswers);
};

console.log(returnPoints(RIGHT_ANSWERS, STUDENT_ANSWERS, compareAnswer));

